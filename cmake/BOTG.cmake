...
ENDMACRO()
#-------------------------------------------------------------------------------
FUNCTION( BOTG_MinimumCompilerVersion compiler lang version )
    BOTG_CompilerMatches( ${compiler} ${lang} found )
    IF( ${found} AND CMAKE_${lang}_COMPILER_VERSION VERSION_LESS ${version} )
        MESSAGE( FATAL_ERROR "compiler=${compiler} for lang=${lang} is required to have minimum version=${version} but found ${CMAKE_${lang}_COMPILER_VERSION}")
    ENDIF()
ENDFUNCTION()
#-------------------------------------------------------------------------------
...
ENDMACRO()
#-------------------------------------------------------------------------------
FUNCTION( BOTG_CompilerMatches compiler lang found_compiler )
   IF( NOT ${compiler} STREQUAL "ANY" )
        STRING(REGEX MATCH "${compiler}" found ${BOTG_${lang}_COMPILER})
        SET(${found_compiler} ${found} PARENT_SCOPE)
        IF( found STREQUAL "" )
            SET(${found_compiler} OFF PARENT_SCOPE)
        ELSE()
            SET(${found_compiler} ON PARENT_SCOPE)
        ENDIF()
    ELSE()
        SET(${found_compiler} ON PARENT_SCOPE)
    ENDIF()
ENDFUNCTION()
#-------------------------------------------------------------------------------
FUNCTION( BOTG_SystemMatches system found_system)
   IF( NOT ${system} STREQUAL "ANY" )
        STRING(REGEX MATCH "${system}" found ${BOTG_SYSTEM})
        IF( found STREQUAL "" )
            SET(${found_system} OFF PARENT_SCOPE)
        ELSE()
            SET(${found_system} ON PARENT_SCOPE)
        ENDIF()
    ELSE()
        SET(${found_system} ON PARENT_SCOPE)
    ENDIF()
ENDFUNCTION()
#-------------------------------------------------------------------------------
FUNCTION( BOTG_CompilerAndSystemMatches compiler lang system found_both)
    BOTG_CompilerMatches( ${compiler} ${lang} found_compiler )
...
MACRO( BOTG_AddCompilerFlags lang compiler system) #list of flags comes at end
    BOTG_CompilerAndSystemMatches( ${compiler} ${lang} ${system} found )
    IF( found )
